...
- `task pm:lock dir=<project-dir>` — generate `pm.lock` from `pm.yaml`/`pm.yml` (runs pm inside the digest-pinned container-factory image; needs only Docker). For non-default file names pass `from=<spec>` and `lock=<lock>` — the manifest path is recorded in the lock metadata, so the file name matters. NEVER write or edit `pm.lock` by hand — ALWAYS generate it with this task.
- `task pm:install` — extract the pm binary (linux/amd64) to `dest=<path>` (default `./bin/pm`). Optional; `pm:lock` does not require it.
