...
```shell
      --check-source-distribution=false
            Enable source distribution URL validation: the URL must exist and point to an archive.  
            Also validates VCS URLs of every component, as --check-vcs does; cannot be combined     
            with --check-vcs-leaf-only
      --check-vcs=false
