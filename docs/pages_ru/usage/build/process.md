...
Послойное кэширование образов является неотъемлемой частью сборочного процесса werf. werf сохраняет и переиспользует сборочный кэш в container registry.
...
4. В момент публикации werf опирается на контентную адресацию стадий: тег стадии вычисляется из дайджеста её содержимого, поэтому одинаковое содержимое всегда отображается в один и тот же тег, а конкурентная публикация идентичного содержимого безопасна для registry. Параллельные сборщики могут собрать один и тот же слой независимо, но публикуемый результат идентичен.
...
По умолчанию werf хранит служебные метаданные (image-metadata, используемые для очистки на основе истории Git, список managed images, метаданные custom-тегов, маркеры отклонённых стадий, записи о выполненных очистках) в основном репозитории (`--repo`) рядом со стадиями образов. При необходимости эти метаданные можно вынести в отдельный репозиторий с помощью `--meta-repo`:

```shell
werf build --repo registry.mycompany.org/project --meta-repo registry.mycompany.org/project-meta
```

Вынесение метаданных из репозитория стадий полезно, когда у стадий и метаданных разные жизненные циклы или паттерны доступа — например, когда репозиторий стадий разделяется между проектами, а метаданные должны оставаться изолированными, либо когда метаданные удобнее держать в более дешёвом или доступном container registry. Команды `werf cleanup` и `werf purge` должны вызываться с тем же значением `--meta-repo`, что и при сборке.

> **ВАЖНО.** Автоматической миграции уже существующих метаданных из основного репозитория в `--meta-repo` не предусмотрено. После включения флага werf читает и пишет метаданные только в `--meta-repo`; метаданные, ранее сохранённые в `--repo`, больше не используются и останутся там как сироты до тех пор, пока не будут удалены вручную. Включайте этот флаг с самого начала жизни проекта.
