...
***Пользовательские стадии*** — это стадии со сборочными инструкциями из конфигурации. Другими словами — это стадии, конфигурируемые пользователем (существуют также служебные стадии, которые пользователь конфигурировать не может). Сборочные инструкции описываются с помощью _shell_.
...
При сборке стадии предполагается, что инструкции стадии будут запускаться в контейнере, основанном на предыдущей собранной стадии или [на базовом образе]({{ "usage/build/stapel/base.html#from" | true_relative_url }}). Такой контейнер будет упоминаться далее как **сборочный контейнер**.

Перед запуском _сборочного контейнера_ werf подготавливает набор инструкций, который зависит от типа стадии и содержит как служебные команды werf, так и пользовательские, указанные в конфигурации `werf.yaml`. Например, среди служебных команд может быть добавление файлов, наложение патчей и т.п.
...
Для платформ `linux/amd64` и `linux/arm64` бинарь werf уже содержит этот Stapel-образ встроенным внутри себя, поэтому werf загружает его напрямую, а не тянет из registry — доступ к сети и `registry.werf.io` в этом случае не требуется, что позволяет работать «из коробки» в закрытых/изолированных окружениях. Если используется другая платформа, либо переопределена ссылка на Stapel-образ (см. ниже), werf, как и прежде, обращается к registry за образом.

Если окружение не может обращаться к этому образу напрямую, например в закрытом контуре или при использовании приватного registry, ссылку на служебный Stapel-образ можно переопределить перед запуском werf:
...
Директива `packages` позволяет декларативно объявлять зависимости от пакетов. werf обрабатывает каждую запись в отдельной стадии `packagesInstall`, которая выполняется до стадии `install`. Если включена [генерация SBOM]({{ "/usage/build/sbom.html" | true_relative_url }}) (`build.sbom.enable: true`), установленные пакеты — вместе с их транзитивными зависимостями — попадают в итоговый SBOM образа.

При включённой генерации SBOM в shell-стадиях отключается сеть, поэтому установить зависимости командами в любой shell-стадии (`beforeInstall`, `install`, `beforeSetup`, `setup`) невозможно — директива `packages` становится единственным каналом установки зависимостей (см. [технические ограничения SBOM]({{ "/usage/build/sbom.html#технические-ограничения" | true_relative_url }})).

Поддерживаются два вида источников пакетов:

- **менеджеры системных пакетов**: `os-pm`;
- **менеджеры языковых пакетов** (файловые): `go-mod`, `python-uv`, `python-pip`, `python-poetry`, `rust-cargo`, `lua-rock`, `javascript-npm`, `javascript-yarn`, `javascript-pnpm`.
...
- `spec` — inline-список пакетов для установки; версия задаётся через `==` (`curl==8.12.1`) или `@` (`curl@8.12.1`), без версии устанавливается версия по умолчанию. Это единственный формат: файл со списком пакетов вместо inline-списка не поддерживается.
- `workdir` для `os-pm` не поддерживается.

Базовый образ должен предоставлять бинарь `pm` в `$PATH` — иначе сборка завершится ошибкой, так как `pm` не будет найден. Переменные окружения `PACKAGES_VERSION` и `REGISTRY` задаются в самих сборочных базовых образах, поэтому обычно задавать их вручную не требуется. `PACKAGES_VERSION` обязательна: werf сохраняет её в образе и указывает в SBOM как квалификатор `containerfactoryversion`, и стадия `packages` завершается ошибкой, если значение неоткуда взять.

Если базовый образ не задаёт эти переменные — например, это `scratch` с импортированным в него `pm`, — передайте их через `env`, сославшись на секрет сборки, если значение не должно попадать в сборочные инструкции (см. [Секреты в packages](#секреты-в-packages)):
...
Объявленный секрет попадает в `pm` только через такую ссылку: werf не подхватывает секрет по совпадению его id с именем переменной. Сборке, которая полагалась на то, что секрет с именем `PACKAGES_VERSION` или `REGISTRY` сам доезжает до `pm`, нужно добавить соответствующую запись в `env` — иначе стадия `packages` завершится ошибкой из-за отсутствующей версии, а `pm` уйдёт на свой реестр по умолчанию.
...
Ссылка разрешается во время работы пакетного менеджера, поэтому секрет не попадает ни в инструкции сборки, ни в дайджест стадии. Попадёт ли он в итоговый образ — зависит от пакетного менеджера: `PACKAGES_VERSION` werf осознанно записывает в образ и в SBOM, поэтому не передавайте туда значение, которое не должно читаться из образа. Ссылка на необъявленный секрет прерывает сборку на этапе разбора конфигурации. Любая другая последовательность вида `%...%` остаётся литералом.
...
Если в сборочном образе нет Yarn, pnpm, uv или Poetry, установите менеджер предыдущей записью `packages`, а в следующей укажите путь к нему в поле `manager`. Сеть доступна только на стадии packages, поэтому все шаги выполняются на ней — включая установку самого npm через `os-pm`, если в сборочном образе нет и Node.js:
...
Записи выполняются в порядке объявления: `pm` устанавливает Node.js с npm, npm устанавливает Yarn, Yarn устанавливает зависимости приложения. Записи с npm поле `manager` не нужно — `pm` устанавливает npm в `PATH`.

Здесь `/tools/package.json` объявляет сам Yarn зависимостью, а `/tools/package-lock.json` фиксирует его версию и хеш целостности. Менеджер устанавливается как обычная зависимость: он попадает в SBOM образа и остается в собранном образе. `manager`, указывающий куда-то еще — голое имя исполняемого файла или путь из сборочного образа, — отклоняется: такой файл выбирает образ, а не конфигурация.
...
_Дайджест пользовательской стадии_ зависит от итогового текста инструкций, т.е. после применения шаблонизатора. Любые изменения в тексте инструкций с учетом применения шаблонизатора Go в _пользовательской стадии_ приводят к пересборке _стадии_. Например, вы используете следующие _shell-инструкции_:
...
Параметр `git.stageDependencies` задаёт, изменения каких файлов приводят к пересборке пользовательских стадий. Синтаксис:
...
Маски `git.stageDependencies` работают совместно с фильтрами `git.includePaths` и `git.excludePaths` (смотри подробнее про них [в соответствующем разделе]({{ "usage/build/stapel/git.html#использование-фильтров" | true_relative_url }})): оба фильтра только сужают набор файлов, которые может захватить маска. Файл считается зависимостью _пользовательской стадии_, если он подпадает под одну из масок `stageDependencies` этой стадии, удовлетворяет фильтру `includePaths` (если фильтр задан) и не удовлетворяет фильтру `excludePaths`. Исключение только вычитает файлы: `excludePaths` никогда не добавляет файлы в зависимости стадии.
...
### Маски незаданных стадий

Маски незаданных стадий werf выводит из заданных. Стадии всегда упорядочены по порядку сборки — _install_ → _beforeSetup_ → _setup_ — независимо от порядка ключей в `werf.yaml`, и маски выводятся отдельно для каждого _git mapping_:

- стадия с явно заданными масками использует именно эти маски;
- явно заданный пустой список (`[]`) означает отсутствие прямой зависимости от файлов Git-репозитория и при этом считается объявлением стадии;
- незаданная стадия **до** последней явно объявленной стадии получает `[]`;
- незаданная стадия **после** последней явно объявленной стадии получает `**/*`;
- если `stageDependencies` не указан вовсе или задан как `{}`, все три стадии получают `**/*`.

Итоговые маски для типовых конфигураций:

| `stageDependencies` в `werf.yaml` | _install_ | _beforeSetup_ | _setup_ |
| --- | --- | --- | --- |
| не указан или `stageDependencies: {}` | `**/*` | `**/*` | `**/*` |
| `beforeSetup: ["src/**/*"]` | `[]` | `src/**/*` | `**/*` |
| `install: ["package.json", "package-lock.json"]` и `setup: ["src/**/*"]` | `package.json`, `package-lock.json` | `[]` | `src/**/*` |
| `install: ["package-lock.json"]` | `package-lock.json` | `**/*` | `**/*` |
| `beforeSetup: []` | `[]` | `[]` | `**/*` |
| `setup: []` | `[]` | `[]` | `[]` |

Например, при неизменных остальных входных данных сборки в конфигурации ниже стадия _install_ пересобирается только при изменении `package-lock.json`, а стадии _beforeSetup_ и _setup_, идущие после последней объявленной стадии, — при любом изменении добавляемых исходников:
...
Чтобы ничего не оставлять выводимым маскам, объявите все три стадии явно:
...
Пустой список отключает только прямую зависимость стадии от файлов Git-репозитория. Зависимость от предыдущих стадий, базового образа, сборочных инструкций и остальных входных данных не меняется: при пересборке более ранней стадии последующие пересобираются тоже.

Маски не создают стадии: _пользовательская стадия_ без сборочных инструкций не создаётся независимо от масок, а непустой набор масок для стадии без сборочных инструкций остаётся ошибкой конфигурации. У стадии _beforeInstall_ нет ключа в `stageDependencies`, и она никогда не зависит от файлов Git-репозитория напрямую.
...
Параметры _CacheVersion_ можно использовать совместно [с шаблонами Go-шаблонизатора]({{ "usage/project_configuration/werf_yaml_template_engine.html" | true_relative_url }}), чтобы определить зависимость _пользовательской стадии_ от файлов, не находящихся в Git-репозитории.
...
По умолчанию werf монтирует сокет SSH-агента во все сборочные контейнеры, что позволяет вашим инструкциям использовать SSH-агент для аутентификации. Это особенно полезно, если ваши команды требуют SSH-доступа, например, для клонирования приватных репозиториев.
