...
Базовый образ должен предоставлять бинарь `pm` в `$PATH` — иначе сборка завершится ошибкой, так как `pm` не будет найден. Переменные окружения `PACKAGES_VERSION` и `REGISTRY` задаются в самих сборочных базовых образах — задавать их вручную не требуется.
