...
Файловые типы запускают команду установки экосистемы внутри контейнера сборки и передают полученный lock-файл в syft для генерации SBOM. Пакетный менеджер должен быть предустановлен в образе сборщика. Для Yarn, pnpm, uv и Poetry укажите обязательную `version`, например `1.22.22`: werf установит выбранный менеджер через npm или pip для стадии packages и удалит его после успешной установки. Версия должна быть корректной версией Semantic Versioning. В образе сборщика должны быть npm для Yarn/pnpm или `python3` с поддержкой `python3 -m venv` для uv/Poetry; выбранный альтернативный менеджер не должен быть установлен заранее.

Для альтернативных менеджеров поле `version` указывается в каждой записи `packages` и не может быть пропущено. Менеджер устанавливается во временное изолированное окружение конкретной директивы и запускается по абсолютному пути; если такой менеджер уже доступен в образе сборщика, werf отклоняет сборку. Временное окружение удаляется только после успешной установки зависимостей по lock-файлу. Для миграции существующего образа удалите предустановленный Yarn, pnpm, uv или Poetry из образа сборщика, добавьте соответствующую `version` в `werf.yaml` и проверьте наличие инструмента bootstrap и поддержки виртуальных окружений Python. Основные менеджеры, например `javascript-npm` и `python-pip`, не принимают поле `version` и сохраняют прежнее поведение.
