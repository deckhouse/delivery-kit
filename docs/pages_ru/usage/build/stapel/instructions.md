...
При включённой генерации SBOM в shell-стадиях отключается сеть, поэтому установить зависимости командой в `shell.install` невозможно — директива `packages` становится единственным каналом установки зависимостей (см. [ограничения SBOM]({{ "/usage/build/sbom.html#ограничения" | true_relative_url }})).

Поддерживаются два вида источников пакетов: менеджер системных пакетов (`os-pm`) и файловые экосистемы (`go-mod`, `python-uv`, `python-pip`, `python-poetry`, `rust-cargo`, `lua-rock`, `javascript-npm`, `javascript-yarn`, `javascript-pnpm`).
...
- `spec` — inline-список пакетов для установки; версия задаётся через `==` (`curl==8.12.1`), без версии устанавливается версия по умолчанию. Это единственный формат: файл со списком пакетов вместо inline-списка не поддерживается.
