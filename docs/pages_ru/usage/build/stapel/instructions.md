...
Здесь `/tools/package.json` объявляет сам Yarn зависимостью, а `/tools/package-lock.json` фиксирует его версию и хеш целостности. Менеджер устанавливается как обычная зависимость: он попадает в SBOM образа и остается в собранном образе. Путь в `manager`, указывающий куда-то еще — например, в сборочный образ, — отклоняется: такой файл выбирает образ, а не конфигурация. Голое имя исполняемого файла (`manager: pip`) допускается: оно ищется в `PATH` образа так же, как значение по умолчанию, и покрывает образы, где исполняемый файл называется иначе.
...
По умолчанию werf отслеживает все файлы (`**/*`), поэтому любое изменение исходников запускает пересборку пользовательских стадий с командами сборки. С помощью параметра `git.stageDependencies` можно сузить набор файлов, изменения которых приводят к пересборке. Синтаксис:
