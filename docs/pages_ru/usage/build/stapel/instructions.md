...
Директива `packages` позволяет декларативно устанавливать бинарные пакеты с помощью пакетного менеджера `pm`. werf устанавливает перечисленные пакеты в отдельной стадии `packagesInstall`, которая выполняется до стадии `install`. Если включена генерация SBOM (`build.sbom.enable: true`), установленные пакеты — вместе с их транзитивными зависимостями — попадают в итоговый SBOM образа.
...
Директива поддерживает следующие опции:

- `type` — тип источника пакетов. Сейчас поддерживается только `os-pm` (пакетный менеджер `pm`).
- `spec` — inline-список имён пакетов для установки. Списки пакетов из файла не поддерживаются.
...
> **Примечание:** синтаксис ansible не доступен для использования при использовании сборочного бэкенда Buildah.
