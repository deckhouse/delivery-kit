...
var _ = ginkgo.DescribeTable("git stageDependencies config to git mapping wiring",
	...
	ginkgo.Entry("an explicitly empty packages list stays declared", `  stageDependencies:
    packages: []`, func(deps map[stage.StageName][]string) {
		gomega.Expect(deps).To(gomega.HaveLen(1))
		gomega.Expect(deps).To(gomega.HaveKeyWithValue(stage.Packages, []string{}))
	}),
)
