...
func PullAndParseImages(ctx context.Context, registry docker_registry.Interface, repo string, mapping map[string]string) ([]*convert.ImageSBOM, error) {
	...
	for imageName, imageDigest := range mapping {
		...
		err := logboek.Context(ctx).Default().
			...
			DoError(func() error {
				sbomRef, err := resolveSBOMReference(ctx, registry, repo, imageDigest, digestToTag)
				if err != nil {
...
func resolveSBOMReference(ctx context.Context, registry docker_registry.Interface, repo, imageDigest string, digestToTag map[string]string) (string, error) {
	tag, ok := digestToTag[imageDigest]
	if !ok {
		return "", fmt.Errorf("no tag found for digest %s in repo %s", imageDigest, repo)
	}

	return fmt.Sprintf("%s:%s-sbom", repo, tag), nil
}
