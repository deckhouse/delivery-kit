...
func (m *StorageManager) FetchStageImage(ctx context.Context, containerBackend container_backend.ContainerBackend, logName string, stageImage *stage.StageImage) (FetchStageInfo, error) {
	...
	shouldFetch, err := m.StagesStorage.ShouldFetchImage(ctx, stageImage.Image)
	if err != nil {
		return FetchStageInfo{}, fmt.Errorf("error checking should fetch image: %w", err)
	}
	if !shouldFetch {
		imageName := m.StagesStorage.ConstructStageImageName(m.ProjectName, stageImage.Image.GetStageDesc().StageID.Digest, stageImage.Image.GetStageDesc().StageID.CreationTs)
	...
	if fetchedImg == nil {
		...
		err := logboek.Context(ctx).Default().LogProcess("Fetching stage %s from %s", logName, m.StagesStorage.String()).
			DoError(func() error {
		...
		if storage.IsErrStageUnavailable(err) {
			...
			logboek.Context(ctx).Error().LogF("Will mark image %s as rejected in the stages storage %s\n", stageImage.Image.Name(), m.StagesStorage.String())
			if err := m.StagesStorage.RejectStage(ctx, m.ProjectName, stageID.Digest, stageID.CreationTs); err != nil {
				return FetchStageInfo{}, fmt.Errorf("unable to reject stage %s image %s in the stages storage %s: %w", logName, stageImage.Image.Name(), m.StagesStorage.String(), err)
			}
...
func (m *StorageManager) CopySuitableStageDescByDigest(ctx context.Context, stageDesc *image.StageDesc, sourceStagesStorage, destinationStagesStorage storage.StagesStorage, containerBackend container_backend.ContainerBackend, targetPlatform string) (*image.StageDesc, error) {
	...
	if destinationStageDesc, err := getStageDesc(ctx, m.ProjectName, *stageDesc.StageID, destinationStagesStorage, m.CacheStagesStorageList, getStageDescOptions{WithLocalManifestCache: m.getWithLocalManifestCacheOption()}); err != nil {
		...
		if sourceStagesStorage.Address() != storage.LocalStorageAddress && destinationStagesStorage.Address() != storage.LocalStorageAddress {
			// The backend-mediated copy does not guarantee digest preservation. When the
			// digest survived, the attached artifacts still describe the copied image and
			// are carried byte-identical. When it changed, a statement about the source
			// digest is not a statement about the destination digest: werf-generated
			// artifacts are regenerated by convergence against the new digest in the same
			// run, and artifacts werf cannot regenerate are left behind.
			if destinationStageDesc.Info.GetDigest() == stageDesc.Info.GetDigest() {
				...
				logboek.Context(ctx).Warn().LogF("WARNING: artifacts attached to stage %s in %s are not carried over: the copy into %s changed the image digest (%s -> %s), so werf-generated artifacts will be regenerated and any user-signed attestations have to be re-issued against the new digest\n",
					stageDesc.StageID.String(), sourceStagesStorage.String(), destinationStagesStorage.String(), stageDesc.Info.GetDigest(), destinationStageDesc.Info.GetDigest())
			}
		}
