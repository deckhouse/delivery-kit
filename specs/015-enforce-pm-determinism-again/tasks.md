...
- [X] T013 [US1] Add missing `pm.lock` validation error in `pkg/build/pm_bom_patcher.go` — when `pm.yaml` exists but `pm.lock` is missing in the commit tree, fail with error: `"pm.lock not found at <path>. Run 'pm lock' in your repository to generate the lock file, commit it, and retry."` (implemented in PM BOMPatcher during build phase, following the same pattern as gomod spec file checking)
