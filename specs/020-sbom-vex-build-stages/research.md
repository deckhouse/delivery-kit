...
- Treat the artifact as a synthetic image stage or image layer: rejected because it changes image semantics and can affect image digest/content.
- Let stages call concrete registry clients directly: rejected because it bypasses the established `StagesStorage` abstraction and makes storage backends inconsistent.
