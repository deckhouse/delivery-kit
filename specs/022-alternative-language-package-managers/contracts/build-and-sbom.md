...
`pkg/build/stage/packages.go` continues to create one network-enabled package stage. `pkg/config.GeneratePackagesCommands` supplies the ordered command sequence. The generated sequence is included in the existing package-stage checksum so changing an alternative-manager version invalidates the package stage.
...
Temporary manager binaries/packages are removed after dependency installation, but project dependencies and manifest/lock files remain available for the existing SBOM scan. No new SBOM format or cataloger is introduced.
