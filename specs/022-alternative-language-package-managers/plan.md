...
7. Add focused Ginkgo/Gomega tests for the switch helper, complete generated wrapper snippets, env propagation to a stub external manager, UUID scope propagation, embedded `mkdir`/`rm` paths, absence of `mktemp`/`grep` and a redundant inner `set -e`, parsing, required/invalid/unsupported-type/valid-SemVer validation cases in a `DescribeTable`, pre-installed-manager rejection, failure ordering, scope cleanup, primary types, and multiple directives; compare each full snippet rather than isolated command lines. Post-bootstrap version-output verification is out of scope.
