...
1. Check the original environment for the manager executable (`yarn`/`yarnpkg`, `pnpm`, `uv`, or `poetry`).
2. Emit one readable `if ... fi` block. In the existing-manager branch, invoke the discovered executable directly and do not clean it up. In the absent-manager branch, create a unique directive-local scope according to the ecosystem: JavaScript uses an npm prefix; Python uses a venv.
3. Inside the absent-manager branch, install and verify the exact manager version, run the existing frozen dependency-install command through the isolated executable, and remove only the temporary scope after success. Do not prepend the temporary scope to a process-global `PATH`.

The primary manager is not bootstrapped for `javascript-npm` or `python-pip`, and those generated commands remain unchanged.

**Rationale:** `PackagesStage` already has network access, command content participates in `PackagesChecksum`, and a single human-readable block preserves failure ordering. Reusing an existing executable avoids unnecessary installation and never mutates or removes pre-existing builder state. A prefix or virtual environment makes a newly installed manager available independently of the project workdir. Absolute executable paths avoid `PATH` leakage between directives. Shell sequencing ensures dependency failure returns the dependency error and prevents successful-install cleanup from masking it.
