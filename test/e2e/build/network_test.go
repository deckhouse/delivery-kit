...
var _ = Describe("Network isolation build", Label("e2e", "build", "network"), func() {
	DescribeTable("should handle network isolation correctly",
		...
		backendEntry("Stapel (Docker): sbom.enable=true enforces --network=none on shell stages (should fail)", networkTestOptions{
			setupEnvOptions:    setupEnvOptions{ContainerBackendMode: "docker", WithLocalRepo: false},
			ExpectError:        true,
			FixturePath:        "network/managed_deps_enforcement",
			NetworkNone:        false,
			ExpectNetworkValue: "none",
		}, Label("stapel", "managed-deps")),
		backendEntry("Stapel (Docker): sbom.enable=false does not enforce --network=none (should succeed)", networkTestOptions{
			setupEnvOptions: setupEnvOptions{ContainerBackendMode: "docker", WithLocalRepo: false},
			ExpectError:     false,
			FixturePath:     "network/stapel",
			NetworkNone:     false,
