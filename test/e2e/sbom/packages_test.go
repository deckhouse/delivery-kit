...
var _ = Describe("SBOM os-pm packages", Label("e2e", "sbom", "packages", "simple"), func() {
	It("verifies licenses, hashes, externalRefs, properties, and dependency graph on a single image", func(ctx SpecContext) {
		setupSbomBuildEnv()
	...
	It("merges parent base-layer os-pm packages into child image SBOM", func(ctx SpecContext) {
		setupSbomBuildEnv()
	...
	It("propagates parent fromImage os-pm packages into child image SBOM (no own packages)", func(ctx SpecContext) {
		setupSbomBuildEnv()
	...
	It("build fails when pm binary is missing in the image despite os-pm packages declared", func(ctx SpecContext) {
		setupSbomBuildEnv()
	...
	It("resolves pm env from referenced build secrets on a scratch base image without own coreutils", func(ctx SpecContext) {
		setupSbomBuildEnv()
