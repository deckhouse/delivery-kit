...
var _ = Describe("SBOM stageDependencies cache invalidation", Label("e2e", "sbom", "stage-deps", "simple"), func() {
	...
	It("regenerates SBOM on package spec changes; uses cache when unchanged", func(ctx SpecContext) {
		setupSbomBuildEnv()
	...
	It("changes to a file listed in git.stageDependencies.packages invalidate the SBOM cache", func(ctx SpecContext) {
		setupSbomBuildEnv()
	...
	It("go.mod change tracked by git.stageDependencies.packages rebuilds the packages stage", func(ctx SpecContext) {
		setupSbomBuildEnv()
	...
	It("switching packages type from os-pm to go-mod regenerates SBOM with new scanner output", func(ctx SpecContext) {
		setupSbomBuildEnv()
