...
var _ = Describe("SBOM python-uv packages", Label("e2e", "sbom", "uv", "simple"), func() {
	DescribeTable("catalogs uv.lock dependencies into the BOM",
		func(ctx SpecContext, testOpts sbomTestOptions) {
			...
			managerAbsentOut := werfProject.RunCommand(ctx, []string{"run", "app", "--", "sh", "-c", "command -v uv >/dev/null 2>&1 && exit 1; for scope in /tmp/werf-packages-*; do test -e \"$scope\" && exit 1; done; exit 0"}, werf.CommonOptions{Envs: builderEnv})
