...
var _ = Describe("SBOM javascript-yarn packages", Label("e2e", "sbom", "yarn", "simple"), func() {
	DescribeTable("catalogs yarn.lock dependencies into the BOM",
		func(ctx SpecContext, testOpts sbomTestOptions) {
			...
			managerAbsentOut := werfProject.RunCommand(ctx, []string{"run", "app", "--", "sh", "-c", "command -v yarn >/dev/null 2>&1 && exit 1; command -v yarnpkg >/dev/null 2>&1 && exit 1; exit 0"}, werf.CommonOptions{Envs: builderEnv})
